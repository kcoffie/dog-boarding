...
**Status:** v2.3 NEARLY DONE — 643 tests (642 pass, 1 pre-existing date-flaky). Commit `ebcb00f` deployed to Vercel. REQ-306 code fix pending (see below).
...
### REQ-300 ✅ — Done (Supabase dashboard config complete)

### REQ-302 ✅ — Done (SQL backfill run, entities decoded)

### REQ-305 ✅ — Done (Mochi Hill Jan 23–26 data verified)

### REQ-303 ✅, REQ-304 ✅ — Verified in UI

### REQ-306 — Millie McSpadden: needs SQL cleanup + code fix

**Root cause discovered:** The sync has no check for `sync_status = 'archived'`. After the manual SQL archived C63QgH5K and deleted its boarding, a sync re-fetched C63QgH5K from the schedule page (it's still visible on the external site), found no boarding by external_id (just deleted), saw C63QgNHs (Mar 4–19) in the overlap check with a *different* external_id → "amended appointment" case → created a new boarding. Will keep recurring on every sync until the external site makes C63QgH5K inaccessible.

**Step 1 — SQL (run in Supabase, clears the re-created boarding):**
```sql
-- sync_appointment is already archived from the first run; just re-delete the boarding
UPDATE sync_appointments SET mapped_boarding_id = NULL
WHERE mapped_boarding_id = (SELECT id FROM boardings WHERE external_id = 'C63QgH5K');
DELETE FROM boardings WHERE external_id = 'C63QgH5K';
```

**Step 2 — Code fix in `src/lib/scraper/sync.js` (prevents recurrence):**

In `runSync`, before the appointment loop, preload all archived external_ids:
```js
// ~line 335, right after seenExternalIds Set is declared
...
Then, in the loop right after the pre-filter block (after the `isKnownNonBoarding` continue, ~line 379):
```js
// Skip appointments already explicitly archived in the DB
...
Commit as: `fix: skip archived appointments during sync to prevent re-creation (#306)`
